...
class FileccController extends GlobalController {
    protected $modelUser;
...
END:
    ...
    public function importDepart()
    {
        ...
        for ($Row = 2; $Row <= $allRow; $Row++) {
            ...
            $dname = $dept_name.'-'.$dept_pid_name;
            if(in_array($dname,$d_arr))
                continue;
            $d_arr[]= $dname;
            ...
            if($id){
                ...
                $new_dept_name[] = ['name'=>$dname,'pid' => 1,'atime' => $t];
...
//        }
        ...
        $excel_id_card = [];
        for ($Row = 2; $Row <= $allRow; $Row++) {
            $d1 = (string)$currentSheet->getCell('B' . $Row)->getValue();
            $d2 = (string)$currentSheet->getCell('C' . $Row)->getValue();
            $nickname = (string)$currentSheet->getCell('D' . $Row)->getValue();
            $id = $currentSheet->getCell('E' . $Row)->getValue();

            if(empty($phone_num) && empty($nickname))
                continue;

            //身份证必填
            if(empty($currentSheet->getCell('E' . $Row)->getValue())){
...
//        $kkk = ['idcard','nickname','fid','dept_id','atime'];
//        foreach($list as $v){
//                if($v['nickname']== '李在生')
//                    var_dump($v);
//        }
...
END:
    ...
    public function sss(){
        for($i=1;$i<1000;$i++)
            $d[] = ['mtime'=>1548086400,'atime'=>1548086400,'uid'=>$i,'step_n'=>rand(1566,15333)];
...
END:
    ...
    public function addExcelUserActRank( &$objExcel, $data=[] ,$title = []) {
        ...
        foreach($data as $value) {
            ...
            $objExcel->getActiveSheet()->setCellValue('c'.$i, $value['idcard']);
